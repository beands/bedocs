...
Blume is a **markdown-first** documentation framework on Astro/Vite: content lives as Markdown/MDX under a content root (default `docs/`), navigation derives from the file tree plus optional `meta.ts` files, `bedocs build` validates frontmatter and duplicate routes, and `bedocs validate` checks links and anchors.
...
5. **Verify.**
   - Run the docs build (`bedocs build` or the repo's documented docs QA) — it validates frontmatter and duplicate routes.
   - Run `bedocs validate` to check internal links and anchors.
