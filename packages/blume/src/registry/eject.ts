...
/** The portable `@source` guess: @beands/bedocs in the project's own node_modules. */
const LOCAL_BLUME_SOURCE = "../../node_modules/blume/src/**/*.{astro,ts,tsx}";

/**
 * The `@source` glob pointing Tailwind at BeDocs's own source, relative to the
 ...
 * hoisted installs (npm/yarn workspaces lift @beands/bedocs into the workspace root's
...
export const blumeSourceGlob = (
  ...
  try {
    ...
    console.warn(
      'bedocs: could not locate the installed @beands/bedocs package; src/generated/app.css keeps its default `@source "../../node_modules/@beands/bedocs/..."` glob. If @beands/bedocs is hoisted elsewhere, point that glob at its install location or BeDocs\'s utility classes will be missing.'
    );
    return LOCAL_BLUME_SOURCE;
  }
};
...
const askFiles = async (
  ...
  if (!(ask?.enabled && !ask.endpoint)) {
    ...
    if (existsSync(endpointPath)) {
      ...
      if (content.startsWith("// Generated by BeDocs. Do not edit.")) {
  ...
  const files: {
    ...
    },
    {
      content: tailwindEntryTemplate({
        ...
        // portable; the source glob resolves the real install location when
