...
![BeDocs — Документация без границ](apps/docs/public/bedocs-cover.png)
...
BeDocs — русскоязычный форк [Blume](https://github.com/haydenbleasel/blume), синхронизированный с версией **1.5.3** и адаптированный для русскоязычных разработчиков.

Платформа поддерживает AI-генерацию структурированной документации через API [crea-ai.ru](https://crea-ai.ru/). Проект развивается компанией [BeandsMedia](https://beands-media.ru/).
...
- **Multi-project** — управляйте несколькими проектами документации через admin-панель. Динамическая навигация, drag-and-drop загрузка.
- **AI-генерация через [crea-ai.ru](https://crea-ai.ru/)** — автоматическое создание структурированной документации из сырых Markdown файлов с помощью Gemini моделей. RAG-анализ, поэтапный план, генерация и пересборка. Выполняется фоновым заданием: потоковый вывод, сохранение прогресса после каждой страницы, автоматическое восстановление после обрыва связи или перезапуска сервера — уже созданные страницы не генерируются заново.
...
Этот репозиторий — монорепо: публикуемый пакет находится в `packages/blume`, `apps/docs` — собственная документация BeDocs, а `admin/` — admin-панель для управления проектами и AI-генерации.
