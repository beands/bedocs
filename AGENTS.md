...
Blume is a zero-config documentation site generator: users drop Markdown/MDX in a folder, and the `blume` CLI generates and drives a **hidden Astro project in `.blume/`** (dev server, static build, search, OG images, theming). `bedocs eject` turns that into a standalone Astro app. Zero-config mode is a permanent, first-class design goal — never treat it as a stepping stone to "real" configuration.
...
| `apps/docs` | The dogfooded docs site (github.com/beands/bedocs). Content lives in `apps/docs/content/docs`. |
...
In `apps/docs`, scripts are the Blume CLI itself: `bedocs dev`, `bedocs build`, `bedocs check`, `bedocs audit`, plus `playwright test` for e2e.
...
- **`.blume/` is shared state** between `bedocs dev` and `bedocs build`. Never run a destructive build or `rm -rf .blume` while a dev server is running against it.
